  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        console.log('BookingRequestCard: Booking acceptance failed (returned false)');
        // Don't show additional error toast since updateBookingStatus already shows one
      }
    } catch (error: any) {
      console.error('BookingRequestCard: Failed to accept booking:', error);
      // Only show error if updateBookingStatus didn't already handle it
      if (!error.handled) {
        toast({
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        console.log('BookingRequestCard: Booking decline failed (returned false)');
        // Don't show additional error toast since updateBookingStatus already shows one
      }
    } catch (error: any) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        // Don't show additional success toast since updateBookingStatus already shows one
        onStatusUpdate?.();
      } else {
        console.log('BookingRequestCard: Booking completion failed (returned false)');
        // Don't show additional error toast since updateBookingStatus already shows one
      }
    } catch (error: any) {
