    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border/50 supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 lg:px-6">
        <div className="flex items-center justify-between h-16">
            ...
            {!user ? (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent 
                  ...
                  sideOffset={8}
                >
                  <div className={`flex items-center p-3 rounded-lg bg-muted/30 ${isRTL ? 'gap-x-reverse gap-3' : 'gap-3'}`}>
                    <ProfilePhotoUpload 
                      currentPhotoUrl={userProfile?.profile_photo_url}
                      userFirstName={userProfile?.first_name}
                      userLastName={userProfile?.last_name}
                    ...
                    <div className="flex flex-col space-y-1">
                      ...
                      <div className={`flex items-center mt-1 ${isRTL ? 'gap-x-reverse gap-1' : 'gap-1'}`}>
                  ...
                  <DropdownMenuItem asChild>
                    <Link to="/profile" className={`flex items-center p-2 rounded-md hover:bg-muted/50 transition-colors ${isRTL ? 'gap-x-reverse gap-2' : 'gap-2'}`}>
                  ...
                  <DropdownMenuItem 
                    onClick={logout}
                    className={`flex items-center p-2 rounded-md text-destructive hover:bg-destructive/10 hover:text-destructive transition-colors ${isRTL ? 'gap-x-reverse gap-2' : 'gap-2'}`}
                  >
          ...
          {/* Mobile Menu */}
          <div className={`flex lg:hidden items-center ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'}`}>
            <LanguageSwitcher />
                    ...
                    {!user ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1">
                          <Link to="/profile">
                            <Button variant="ghost" className={`w-full justify-start ${isRTL ? 'gap-x-reverse gap-2' : 'gap-2'}`}>
                              <User className="h-4 w-4" />
                              {t('profile')}
                            </Button>
                          </Link>
                          <Button 
                            onClick={logout} 
                            variant="ghost"
                            className={`w-full justify-start text-destructive hover:bg-destructive/10 hover:text-destructive ${isRTL ? 'gap-x-reverse gap-2' : 'gap-2'}`}
                          >
                            <LogOut className="h-4 w-4" />
                            {t('logout')}
                          </Button>
